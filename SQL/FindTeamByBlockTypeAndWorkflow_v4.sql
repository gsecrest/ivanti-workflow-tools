...
-- WorkflowDefinitionRecID is used to join ServiceReqFulfillmentPlan.WorkflowId
SELECT
    wt.Name  AS WorkflowName,
    wf.RecID AS WorkflowDefinitionRecID,
    wf.DefVersion,
    CAST(
...
LEFT JOIN ServiceReqFulfillmentPlan fp
    ON UPPER(fp.WorkflowId) = UPPER(b.WorkflowDefinitionRecID)
...
FROM #TaskBlocks tb
LEFT JOIN ServiceReqFulfillmentPlan fp
    ON UPPER(fp.WorkflowId) = UPPER(tb.WorkflowDefinitionRecID)
LEFT JOIN FusionLink fl
    ON fl.TargetID = fp.RecId
    AND fl.RelationshipName = 'ServiceReqTemplateAssociatedServiceReqFulfillmentP'
LEFT JOIN ServiceReqTemplate srt
